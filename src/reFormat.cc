...
void load_format_tag(xmlTextReaderPtr format_reader, format &file_format) {
  ...
  if (xmlTextReaderIsEmptyElement(format_reader) != 1) {
    ...
    while (ret == 1 && !(tagType == XML_READER_TYPE_END_ELEMENT && tagName == L"format-tag")) {
      // DTD definizioekin zer egin behar den definitu behar dugu.
      if (tagName== L"#cdata-section") tag.tag += XMLParseUtil::stows(xmlc2s(xmlTextReaderValue(format_reader)));
...
void load_open_close_tag(xmlTextReaderPtr format_reader, format &file_format) {
  ...
  if (xmlTextReaderIsEmptyElement(format_reader) != 1) {
    ...
    while (ret == 1 && !(tagType == XML_READER_TYPE_END_ELEMENT && tagName == L"open-tag")) {
      if (tagName== L"#cdata-section") tag.open.tag += XMLParseUtil::stows(xmlc2s(xmlTextReaderValue(format_reader)));
  ...
  if (xmlTextReaderIsEmptyElement(format_reader) != 1) {
    ...
    while (ret == 1 && !(tagType == XML_READER_TYPE_END_ELEMENT && tagName == L"close-tag")) {
      if (tagName == L"#cdata-section") tag.close.tag += XMLParseUtil::stows(xmlc2s(xmlTextReaderValue(format_reader)));
...
format load_format(xmlTextReaderPtr format_reader){
  ...
  if (!(ret ==1 && tagName == L"format" && tagType == XML_READER_TYPE_ELEMENT)) {
    wcerr << L"ERROR: invalid document: found <" << tagName << L"> when <format> was expected..." << endl;
    exit(-1);
  }
  ...
  while (ret == 1 && tagName != L"format") {
    if (tagName == L"empty-tag" && tagType == XML_READER_TYPE_ELEMENT) load_empty_tag(format_reader, file_format);
    if (tagName == L"format-tag" && tagType == XML_READER_TYPE_ELEMENT) load_format_tag(format_reader, file_format);
    if (tagName == L"open-close-tag" && tagType == XML_READER_TYPE_ELEMENT) load_open_close_tag(format_reader, file_format);
...
int get_min_alloc(wstring alloc) {
  if (alloc.find(L",") == wstring::npos) return watoi(alloc.c_str());
...
wstring get_tags(format &file_format, const vector<int> &allocs){
  ...
  */
  ...
  for (size_t i=0; i<output_tags.size(); i++) {
    ...
    //wcerr << L"\t" << output_tags[i].alloc << L"," << output_tags[i].order << endl;
  }
...
int main(int argc, char *argv[])
{
  ...
  while (ret == 1 and tagName == L"SENTENCE") {
    ...
    while (curSentence != sentence.end()) {
	  ...
	  if (form != L"." && form != L"," && form != L":" && form != L";" && form != L"?" && form != L"!" && form != L")")
            wcout << L" ";
        ...
        if (form == L"." || form == L"?" || form == L"!") upperCase=true;
