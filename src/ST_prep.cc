...
vector<wstring> procCHUNK(xmlTextReaderPtr reader, wstring &attributesFromParent, vector<wstring> &chunk_attributes, wstring sentenceref, int sentencealloc, config &cfg) {
      ...
      if (cfg.first_case || chunk_cases[i].size() == 1) {
        if (chunk_cases[i][0] != L"" && chunk_cases[i][0] != L"[ZERO]") {
          ...
          if (chunk_cases[i].size() != 1 && cfg.DoPrepTrace) {
            wcerr << sentenceref << L":" << alloc << L":" << prep << L" ANBIGUOA GELDITU DA (";
            for(size_t j=0;j<chunk_cases[i].size();j++) {
              wcerr << chunk_cases[i][j] << L" ";
            }
            wcerr << L") HIZTEGIKO LEHENENGOA AUKERATUKO DA: " << chunk_cases[i][0] << endl << endl;
          }
...
wstring procSENTENCE (xmlTextReaderPtr reader, config &cfg) {
  ...
  while (ret == 1 and tagName == L"CHUNK") {
    ...
    for (size_t i=0; i < child_attributes.size(); i++) {
      ...
      if ((!cfg.first_case && chunk_cases[i].size() != 1) || chunk_cases[i][0] == L"" || chunk_cases[i][0] == L"[ZERO]")
        tree += L"<CHUNK" + write_xml(child_attributes[i]) + L">\n" + subTree[i];
      else {

        if (chunk_cases[i].size() != 1 && cfg.DoPrepTrace) {
          wcerr << ref << L":" << alloc << L":" << prep << L" ANBIGUOA GELDITU DA (";
          for(size_t j=0;j<chunk_cases[i].size();j++) {
            wcerr << chunk_cases[i][j] << L" ";
          }
          wcerr << L") HIZTEGIKO LEHENENGOA AUKERATUKO DA: " << chunk_cases[i][0] << endl << endl;
